(function () {
  ...
  function renderHomePage() {
    ...
    page.innerHTML =
      '<div class="home-header nav-header" style="background:var(--blue);padding:0 20px 20px">' +
  ...
  function renderBookingPage() {
    ...
    page.innerHTML =
      ...
      calendar.html +
      "</div></div>" +
      '<div><div class="field-label">' + t("timeLabel") + '</div><div class="time-input-wrap time-select-wrap"><span>' +
      icon("clock") +
      '</span><select class="picker-select time-select" onchange="setBookingField(\'time\', this.value, false)">' +
      TIMES.map(function (time) {
        const busy = hasConflict({ ...state.bookingDraft, time: time, editingId: state.bookingDraft.editingId });
        const selected = state.bookingDraft.time === time;
        return '<option value="' + time + '" ' + (selected ? "selected " : "") + (busy && !selected ? "disabled" : "") + ">" + formatTime(time) + "</option>";
      }).join("") +
      '</select></div></div><div><div class="field-label">' + t("notesLabel") + '</div><textarea class="picker-textarea" oninput="setBookingField(\'notes\', this.value, false)" placeholder="' + t("notesPlaceholder") + '">' +
  ...
  function updateStatusBar() {
    ...
    const element = document.querySelector(".status-time");
